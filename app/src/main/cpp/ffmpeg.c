...
static void log_callback_android_pp(void *ptr, int level, const char *fmt, va_list vl)
{	
...
//int main(int argc, char **argv)
{
    ...
    if(argc>1 && !strcmp(argv[1], "-d")){
        run_as_daemon=1;
        av_log_set_callback(log_callback_android_pp);
        argc--;
        argv++;
    }
	av_log_set_callback(log_callback_android_pp);
